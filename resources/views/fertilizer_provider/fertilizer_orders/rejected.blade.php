...
@section('content')
<h1 class="text-2xl font-bold mb-4">Rejected Fertilizer Orders</h1>
    ...
    <tbody>
        @forelse($rejectedOrders as $order)
            <tr>
                ...
                <td class="border p-2">{{ \Carbon\Carbon::parse($order->creation_date)->format('Y-m-d H:i') }}</td>
                <td class="border p-2">
                    <a href="{{ route('fertilizer_orders.edit', $order->id) }}" class="text-blue-600 hover:underline mr-2">Edit</a>
